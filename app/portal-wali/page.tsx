...
export default function PortalWali() {
  const [activeTab, setActiveTab] = useState<"tagihan" | "infaq" | "jurnal" | "tabungan" | "ppdb">("tagihan");
    ...
    <div className="min-h-screen bg-slate-100 p-4">
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
          <label className="block text-xs font-semibold text-slate-600 mb-1">Pilih Data Anak / Siswa:</label>
          <select className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-slate-50">
            <option>-- Pilih Siswa (T.A. 2026/2027) --</option>
          </select>
        </div>
        ...
        <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200 flex flex-wrap gap-1">
          <button
            ...
            className={`flex-1 min-w-[110px] py-2 text-xs font-semibold rounded-xl transition ${
        ...
        {activeTab === "tagihan" && (
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 space-y-4">
            <h2 className="text-sm font-bold text-slate-800 border-b pb-2">Informasi Tagihan & SPP Bulanan</h2>
            <div className="text-xs text-slate-500 bg-slate-50 p-6 rounded-xl border border-slate-200 text-center">
              Belum ada tagihan SPP aktif untuk siswa yang dipilih.
            </div>
          </div>
        )}

        {/* Tab 2: Infaq Belajar & Pembayaran */}
        {activeTab === "infaq" && (
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 space-y-4">
            <h2 className="text-sm font-bold text-slate-800 border-b pb-2">Pembayaran Infaq Belajar & Kegiatan</h2>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Nominal Infaq / Pembayaran (Rp)</label>
                <input
                  type="number"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`py-2 text-xs font-semibold rounded-lg border ${
                      metodeBayar === "qris" ? "bg-emerald-50 border-emerald-600 text-emerald-800" : "border-slate-200 text-slate-600"
                    }`}
                  >
                    📱 QRIS Instan
                  </button>
                  <button
                    ...
                    className={`py-2 text-xs font-semibold rounded-lg border ${
                      metodeBayar === "transfer" ? "bg-emerald-50 border-emerald-600 text-emerald-800" : "border-slate-200 text-slate-600"
                    }`}
                  >
                    🏦 Transfer Bank
                  </button>
        ...
        {activeTab === "ppdb" && (
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 space-y-4">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                alert("Pendaftaran PPDB berhasil dikirim! Data akan diverifikasi oleh pihak TU.");
              }}
              className="space-y-3"
            >
              <div>
